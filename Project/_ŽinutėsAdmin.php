...
<?php
	...
	if ($userlevel == 3) 
	{
    		echo "<form method='POST' action='_ŽinutėsAdmin.php'> 
       		...
       		<br>
       		<label for='receiver'>Rinktis gavėją:</label>";
		echo "<select name='receiver' id='receiver'>";
			
		$sql = "SELECT DISTINCT sender FROM " . TBL_MESSAGES . " WHERE sender != '" . mysqli_real_escape_string($db, $_SESSION['user']) . "'";
		$result = mysqli_query($db, $sql);

		if (!$result || mysqli_num_rows($result) < 1)
		{
    			echo "<option value='' disabled>Nėra galimų gavėjų</option>";
		} 
		else 
		{
    			while ($row = mysqli_fetch_assoc($result)) 
			{
        			echo "<option value='" . htmlspecialchars($row['sender']) . "'>" . htmlspecialchars($row['sender']) . "</option>";
   			}
		}
		echo "</select><br>
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	elseif (strlen($message) < 3) {
		...
		if ($result) {
       	 		echo "Išsaugota.";
			header("Location: _ŽinutėsAdmin.php");   
        		exit();
    		} 
		else {
